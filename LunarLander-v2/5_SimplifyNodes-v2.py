...
# First Case Node
clf0 = make_pipeline(StandardScaler(), LinearSVC(random_state=0, tol=1e-4, penalty='l1', loss='squared_hinge', dual=False, C=0.0005))
#clf0 = make_pipeline(LinearSVC(random_state=0, tol=1e-4, penalty='l1', loss='squared_hinge', dual=False, C=0.01))
clf0.fit(x, y)
print(clf0.named_steps['linearsvc'].coef_)
print(clf0.score(x, y))

#if (clf0.named_steps['linearsvc'].coef_[0] == 0).any():
#    print("HI")
#    clf0.named_steps['linearsvc'].coef_[0] = n0
#    clf0.named_steps['linearsvc'].intercept_[0] = c0
#print(clf0.score(x, y))

# True Case Node
clf1 = make_pipeline(StandardScaler(), LinearSVC(random_state=0, tol=1e-8, penalty='l1', loss='squared_hinge', dual=False, C=0.001, max_iter=3000))
#clf1 = make_pipeline(LinearSVC(random_state=0, tol=1e-4, penalty='l1', loss='squared_hinge', dual=False, C=0.01))
clf1.fit(x1, y1)
print(clf1.named_steps['linearsvc'].coef_)
...
# False Case Node
clf2 = make_pipeline(StandardScaler(), LinearSVC(random_state=0, tol=1e-8, penalty='l1', loss='squared_hinge', dual=False, C=0.01, max_iter=3000))
#clf2 = make_pipeline(LinearSVC(random_state=0, tol=1e-1, penalty='l1', loss='squared_hinge', dual=False, C=0.01))
clf2.fit(x2, y2)
print(clf2.named_steps['linearsvc'].coef_)
...
def new_program(obs):
    obs = [obs.tolist()]
    if np.dot(n0, obs[0]) <= c0: #clf0.predict(obs)[0]:
